...
public class Stage : Game {
   ...
   // Terrain Following Lerp
   protected bool lerpFlag = false; // Terrain following set to false - added by David Kopp
   // Lights
   ...
   protected Texture2D background; // Texture for setting the background to stars and space - added by David Kopp
   protected Treasure treasure; // Treasure object to create, place, and interact with treasures - added by David Kopp
   protected Pack pack; // Added by David Kopp
   protected int maxDistanceBetweenGraphNodes = 25; // Added by David Kopp
   protected NavGraph graph; // Added by David Kopp
   ...
   public bool YonFlag {
    ...
    /// <summary>
    /// Enables and Disables Terrain Following Lerp - by David Kopp.
    /// </summary>
    ...
    /// <summary>
    /// Allows for NPAgent and Player to interact with the Treasure Class - by David Kopp.
    /// </summary>
	public Treasure Treasure {
		get { return treasure; }}

    /// <summary>
    /// Finds the winner of the treasure game when all treasures are tagged.
    /// Return " "; means the game is not over yet. - by David Kopp
    /// </summary>
	public String Winner {
		get {
			if (treasure.GameOver) {
				if (npAgent.Score > player.Score) { 
					return "Chaser!";
				} else if (npAgent.Score < player.Score) { 
					return "Evader!";  
				} else if (npAgent.Score == player.Score) {
					return "Tie!";
				}
   ...
   protected override void LoadContent() {
      ...
      for (int i = 5; i < 30; i++) inspector.setInfo(i, "  ");
      ...
      npAgent = new NPAgent(this, "Evader",
         new Vector3(490 * spacing, terrain.surfaceHeight(490, 450), 450 * spacing),
				new Vector3(0, 1, 0), 0.0f, "magentaAvatarV6", treasure);  // facing +Z
      ...
      Components.Add(npAgent);
		...
		// create a Pack of 6 dogs centered at (450, 500) that is leaderless
			pack = new Pack(this, "dog", "dogV6", 8, 450, 430, player.AgentObject); // Modified by David Kopp
		Components.Add(pack);
      ...
      Components.Add(m3d);
		// create 20 clouds (Removed clouds by David Kopp)
		...
		background = Content.Load<Texture2D>("stars"); // Initialized background to texture stars - added by David Kopp
		treasure = new Treasure(this, "treasure", "pinkTreasure"); // Initialize treasure to model pinkTreasure - by David Kopp
        Components.Add (treasure); // Add pinkTreasure models to the componets list - by David Kopp
		setInfo(20, String.Format("Treasure Score Board")); // Set the string for the title of the treasure score board - by David Kopp
		setInfo(25, String.Format("Dog Packing Information"));
  	...
  	/// <summary>
  	/// This method starts the creation of the navgraph. First it stamps out the wall pieces.
  	/// </summary>
    private void createGraph() {

   		// find all collidables add them to graph to remove them later. (thus removing the Collision nodes
		foreach (Object3D obj in collidable) {
			if (obj.Name.Contains("wall") || obj.Name.Contains("temple")) { 
				int xObj = (int) obj.Translation.X / spacing;
				int zObj = (int) obj.Translation.Z / spacing;
				
				graph.insertNavNode(new NavNode(new Vector3((xObj-1) * spacing, terrain.surfaceHeight(xObj - 1, zObj - 1),
					(zObj - 1) * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3(xObj * spacing, terrain.surfaceHeight(xObj, zObj - 1),
					(zObj - 1) * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3((xObj + 1) * spacing, terrain.surfaceHeight(xObj + 1, zObj - 1),
					(zObj - 1) * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3((xObj - 1) * spacing, terrain.surfaceHeight(xObj - 1, zObj),
					zObj * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3(xObj * spacing, terrain.surfaceHeight(xObj, zObj),
					zObj * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3((xObj + 1) * spacing, terrain.surfaceHeight(xObj + 1, zObj),
					zObj * spacing), NavNode.NavNodeEnum.COLLISIONPOINT));
				graph.insertNavNode(new NavNode(new Vector3((xObj - 1) * spacing, terrain.surfaceHeight(xObj - 1, zObj + 1),
		...
		// Now we start the stamping of the scene. Because of the stampsize choosen I had to manually stamp two sections
		// so that the wall would be connected by edges correctly later.
		bool noObject;
		for (int x = 3; x <= 508; x += 23) {
			for (int z = 3; z <= 508; z += 23) {
				// Used a rectangle to make it easier to see if a wall or temp is in the area that is going to be stamped.
				Rectangle stamper = new Rectangle(x * spacing, z * spacing, 23 * spacing, 23 * spacing);
				Rectangle collidableObj;
				noObject = true;

				foreach (Object3D obj in collidable) {
					if (obj.Name.Contains("wall") || obj.Name.Contains("temple")) {
						collidableObj = new Rectangle((int) (obj.Translation.X - obj.ObjectBoundingSphereRadius), 
							(int) (obj.Translation.Z - obj.ObjectBoundingSphereRadius), (int) (2 * obj.ObjectBoundingSphereRadius),
							(int) (2 * obj.ObjectBoundingSphereRadius));
							if (stamper.Contains(collidableObj)) {
							fineStamp(x, z);
							noObject = false;
							//Console.WriteLine(stamper.Contains(collidableObj).ToString()); // Test purposes
							break;
						}
					}
				}
				// Needed to manually add because the fine stamping was on the edge of the wall and
				// I needed more points around the left back of the walls. - Added by David Kopp
				if ((x == 417 && z == 440) || (x == 417 && z == 463)) {
				fineStamp(x, z);
				} else if (noObject) {
					normStamp(x,z);
				}
		...
		// This next methods removes the collisionNodes, creates the adjacency list, and passes the graph to the npagent.
		graph.removeCollisionNodes();  
		graph.createAdjacentGraph();
		npAgent.setGraph(graph);
		//graph.displayNodeKeys(); // test purposes - By David Kopp	
		//graph.displayAllAdjagency();	// test purposes - By David Kopp
	}

	/// <summary>
	/// This is the normal stamp size as long as there is no object detected within it. 23 vertices long with a offset of
	/// 3550.9f this larger reach allows for all the correct nodes to be connected by an edge.
	/// </summary>
	/// <param name="x">The x coordinate.</param>
	/// <param name="z">The z coordinate.</param>
	private void normStamp(int x, int z) {
		int x2 = x + 23;
		int z2 = z + 23; 
		int xMid = x + (x2 - x) / 2;
		int zMid = z + (z2 - z) / 2;  
		float diagOffset = 3550.9f;
		
		graph.insertNavNode(new NavNode(new Vector3(x * spacing, terrain.surfaceHeight(x, z), z * spacing), 
			NavNode.NavNodeEnum.WAYPOINT, diagOffset)); // UL Node
		graph.insertNavNode(new NavNode(new Vector3(x2 * spacing, terrain.surfaceHeight(x2, z), z * spacing), 
			NavNode.NavNodeEnum.WAYPOINT, diagOffset)); // UR Node
		graph.insertNavNode(new NavNode(new Vector3(x * spacing, terrain.surfaceHeight(x, z2), z2 * spacing), 
			NavNode.NavNodeEnum.WAYPOINT, diagOffset)); // LL Node
		graph.insertNavNode(new NavNode(new Vector3(x2 * spacing, terrain.surfaceHeight(x2, z2), z2 * spacing), 
			NavNode.NavNodeEnum.WAYPOINT, diagOffset)); // LR Node
		graph.insertNavNode(new NavNode(new Vector3(xMid * spacing, terrain.surfaceHeight(xMid, zMid), zMid * spacing), 
			NavNode.NavNodeEnum.WAYPOINT, diagOffset));	// Mid Node		
	} // Tested	

	/// <summary>
	/// This is the fine stamping for when the larger stamp has an object in it. It stamps all vertices one by one in the
	/// 23x23 stamp. The walls are already tagged so those nodes can not be duplicated in the graph as waypoints.
	/// </summary>
	/// <param name="x">The x coordinate.</param>
	/// <param name="z">The z coordinate.</param>
	private void fineStamp(int x, int z) {
		//Console.WriteLine(x + ", " + z); // testing purposes
		int xhalt = x + 23;
		int zhalt = z + 23; 
		float diagOffset = 160.9f;//distance(xMid, xhalt, zMid, zhalt) * 150;  
		for (int x1 = x; x1 <= xhalt; x1++) {
			for (int z1 = z; z1 <= zhalt; z1++) {
					graph.insertNavNode(new NavNode(new Vector3(x1 * spacing, terrain.surfaceHeight(x1, z1), z1 * spacing), 
						NavNode.NavNodeEnum.WAYPOINT, diagOffset));
			}
		}		
	} // Tested

	/// <summary>
	/// Simple euclidean distance calculation - never leave home without it.
	/// </summary>
	/// <param name="x1">The first x value.</param>
	/// <param name="x2">The second x value.</param>
	/// <param name="z1">The first z value.</param>
	/// <param name="z2">The second z value.</param>
	public float distance(int x1, int x2, int z1, int z2) {
...
//				Console.WriteLine ("test");
			inspector.setScoreBoard(npAgent.Score, player.Score, Winner); // Update the treasure score board in inspector - Added by David Kopp
			inspector.setFlockingBoard(pack.PackingValue); // Update the flocking / packing board in inspector - Added by David Kopp
			}
	  ...
	  // toggle Lerp on or off by hitting L - by David Kopp
			...
			} else if (keyboardState.IsKeyDown (Keys.D) && !oldKeyboardState.IsKeyDown (Keys.D)) { // Toggles the dog info pane. - New by David Kopp**
				inspector.ShowFlocking = !inspector.ShowFlocking;
				inspector.ShowHelp = !inspector.ShowHelp;
			} else if (keyboardState.IsKeyDown (Keys.P) && !oldKeyboardState.IsKeyDown (Keys.P)) { // Increases the probability number 0, .33, .66, .99 in order and then rolls back - New by David Kopp**
				int packingProbIndex = pack.ProbabilityIndex;
				if (packingProbIndex < 3) { pack.ProbabilityIndex = packingProbIndex + 1; }
				else { pack.ProbabilityIndex = 0; }
			}
