...
public class Inspector {
   ...
   private const int scoreBase = 20;     // Begining index of the treasure board text - Added by David Kopp
   private const int flockingBase = 25;     // Begining index of the flocking board text - Added by David Kopp
   /// </summary>
   ...
   private bool showTreasureBoard = false; // initially don't show treasure board - Added by David Kopp
   private bool showFlocking = false; // initally don't show flocking info - Added by David Kopp
   ...
   /// <summary>
   /// Set the boolean flag to show the treasure score board or not - Added by David Kopp
   /// </summary>
   public bool ShowTreasureBoard {
   get { return showTreasureBoard; }
   set { showTreasureBoard = value; }}

   public bool ShowFlocking {
   get { return showFlocking; }
   set { showFlocking = value; }}
   ...
   public void setMatrices(string label1, string label2, Matrix m1, Matrix m2) {
      ...
      infoString[infoBase++] = 
        ...
        /// <summary>
        /// setScoreBoard displays the treasure score board text in the inspector pane.
        /// Shows the score of both Chaser and Evader. Also will show the winner after all
        /// treasures have been tagged. Winner can be Evader, Chaser, or Tie. If the string
        /// winner is the string " " then the game is not over yet and thus no winner is displayed.
        /// By David Kopp
        /// </summary>
        /// <param name="npAgent"> Integer Score of the npAgent aka Chaser</param>
        /// <param name="player"> Integer Score of the player aka Evader</param>
        /// <param name="winner"> String of the name of the winner can be Evader, Chaser, Tie!, or " "</param>
		public void setScoreBoard(int npAgent, int player, String winner) {
			infoBase = scoreBase;
			infoString [infoBase++] = string.Format ("                                              Treasure Score Board");
			infoString [infoBase++] = string.Format ("                                            Chaser =  {0,0} | Evader = {1,0}", npAgent, player);
			if (!winner.Equals (" ") && !winner.Equals ("Tie!")) {
				infoString [infoBase++] = string.Format ("                                              The Winner is {0,0:s}", winner);
			} else if (!winner.Equals (" ") && winner.Equals ("Tie!")) {
				infoString [infoBase++] = string.Format ("                                                  It is a {0,0:s}", winner);
			}
	}
		/// <summary>
		/// Sets the flocking board. - Added by David Kopp
		/// </summary>
		/// <param name="probability">Probability.</param>
		public void setFlockingBoard(double probability) {
			infoBase = flockingBase;
			infoString [infoBase++] = string.Format ("                                           Dog Packing Information");
			infoString [infoBase++] = string.Format ("                                      Current probability of packing: " + probability);	
		}
   ...
   public void Draw(SpriteBatch spriteBatch) {
      ...
      spriteBatch.Draw(infoBackground, new Vector2(0, 0), Color.White);
         ...
         for (int i = 0; i < InfoPaneSize; i++) {
			...
			} else if (showTreasureBoard) { // show treasure board strings 20..24 - added by David Kopp
				infoBase = scoreBase;
				for (int i = 0; i < InfoPaneSize; i++) {
					FontOrigin = infoFont.MeasureString (infoString [infoBase + i]);
					spriteBatch.DrawString (infoFont, infoString [infoBase + i], FontPos [i], fontColor);
				}
			} else if (showFlocking) { // show flocking info strings 25..30 - Added by David Kopp
				infoBase = flockingBase;
				for (int i = 0; i < InfoPaneSize; i++) {
					FontOrigin = infoFont.MeasureString (infoString [infoBase + i]);
					spriteBatch.DrawString (infoFont, infoString [infoBase + i], FontPos [i], fontColor);
